...
public class LoginController {
	...
	public String login(@Valid LoginForm loginForm, BindingResult result,
		...
		String successUrl = loginUserType == ShareHelper.TYPE_MANAGER ? "/manager/top"
				: "/student/lectureList"; // 認証成功時リダイレクト先
		String notSuccessUrl = loginUserType == ShareHelper.TYPE_MANAGER ? "/loginManager"
				: "/loginStudent"; // 認証失敗時リダイレクト先
		...
		/*
		 * case ShareHelper.TYPE_MANAGER: checkResult =
		 * loginService.isManager(loginForm.getId(), hashedPassword); break;
		 * case ShareHelper.TYPE_STUDENT: checkResult =
		 * loginService.isStudent(loginForm.getId(), hashedPassword); break;
		 */
		}

		redirectUrl.append(checkResult ? successUrl : notSuccessUrl);
		if (!checkResult)
			attribute.addFlashAttribute("error", "ユーザが存在しません");
