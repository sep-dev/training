...
public class StudentController {
	...
	public ModelAndView lectureList() {
		...
		mav.addObject("lectureList",
				studentService.getTodayLectureList(studentId)); // 一時的に0を入れているだけ
	...
	public String attendanceAddConfirm(LectureAttendancePK lectureAttendancePK,
			Model model) {
		model.addAttribute("today", ShareHelper.getToday());
		model.addAttribute(lectureService.findByLectureId(lectureAttendancePK
				.getLectureId()));
	...
	public String attendanceAdd(LectureAttendancePK id, Model model) {
		if (!studentService.addLectureAtttendance(id))
			return "redirect:lectureList"; // 失敗画面へ
	...
	public String studentPasswordUpdate(@Validated PasswordEditForm editForm,
			BindingResult result, Model model) {
		if (result.hasErrors())
			return "studentPassEdit";
		boolean updateResult = studentService.updateStudentPassword(
				editForm.getStudentId(), editForm.getNewPassword());
		if (updateResult) {
	...
	public String attendanceDataSearch(
			@ModelAttribute SearchAttendancePastDataForm sapd, Model model) {
		...
		if (sapd.getStartDate().equals(""))
			sapd.setStartDate(studentService.getMinDate(studentId));
		if (sapd.getEndDate().equals(""))
			sapd.setEndDate(ShareHelper.formatHyphenDate(new Date()));

		model.addAttribute(sapd); // 検索条件をそのまま渡す
		model.addAttribute("pastDataList",
				studentService.getAttendancePastData(studentId, sapd));
	...
	public String downloadCSV(SearchAttendancePastDataForm sapd, Model model) {
		...
		model.addAttribute("studentName", studentService
				.studentFindByStudentId(studentId).getStudentName());
		model.addAttribute("createDate",
				ShareHelper.formatJapaneseDate(new Date()));
		model.addAttribute("startDate", sapd.getStartDate());
		model.addAttribute("endDate", sapd.getStartDate());
		model.addAttribute("lessonName", studentService.getSearchLessonMap()
				.get(sapd.getLesson_id()));
		model.addAttribute("hour",
				studentService.getSearcHourMap().get(sapd.getHour()));
		model.addAttribute("pastDateList",
				studentService.getAttendancePastData(studentId, sapd));
	...
	public String search(HttpServletRequest request, Model model) {
		...
		List<Student> list = repository
				.findByStudentNameLikeOrStudentAddressLike("%" + param + "%",
						"%" + param + "%");
		model.addAttribute("datalist", list);
		return "/studentList";
	}
