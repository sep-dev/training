...
public class AttendanceManagementDaoImpl implements AttendanceManagementDao {
        ...
        List<TodayLecture> list = jdbcTemplate.query(this.getLectureListQuery(), new RowMapper<TodayLecture>(){
            ...
            public TodayLecture mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                if(rs.getInt("attendance_state") == 0){
                    Time startTime = hourList.get(lecture.getHour()-1).getStartTime(); //開始時刻を取得
    ...
    public List<AttendancePastData> getAttendancePastData(Integer studentId, SearchAttendancePastDataForm sapdf) {
        /*
         * クエリにバインドする値を格納する。
         * クエリ取得関数内で引数の有無でクエリを変更しているので、バインドする値の数も可変にする必要がある
         */
        ...
        if(!sapdf.getLesson_id().equals("0")) bindList.add(sapdf.getLesson_id().toString()); //科目ID追加
        if(!sapdf.getHour().equals("0")) bindList.add(sapdf.getHour().toString()); //時限追加

        List<AttendancePastData> list = jdbcTemplate.query(
                getAttendancePastDataQuery(studentId, sapdf), bindList.toArray(),
                new RowMapper<AttendancePastData>(){
                    @Override
                    public AttendancePastData mapRow(ResultSet rs, int rowNum) throws SQLException {
    ...
    private String getAttendancePastDataQuery(Integer studentId,SearchAttendancePastDataForm sapdf){

        StringBuilder query = new StringBuilder();
        query.append("SELECT B.lecture_date AS date,B.lecture_hour AS hour ,C.lesson_name AS lesson_name ")
        .append("FROM ")
        ...
        query.append(") AS B ");
        query.append("ON A.lecture_id = B.lecture_id ")
        .append("INNER JOIN ")
        .append("lessons AS C ")
    ...
    public String getMinDate(Integer studentId){
        String query =
                ...
                + ")";
        return jdbcTemplate.queryForMap(query, studentId).get("minDate").toString();
    }
