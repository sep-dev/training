...
<html>
	...
	<body>
		...
		<c:choose>
				...
				<table>
					<form:form modelAttribute="formModelRadio" action="Addressupdate">
						<c:forEach var="obj" items="${list1}" varStatus="status">
							<tr>
							<td><form:radiobutton path="radio1" value="${obj.id}"/></td>
						...
						<input type="button" value="会員登録" onClick="location.href='Addressup'">
					</form:form>
				</table>
			</c:when>
			...
			<c:when test="${change ==5}">
				名前:<c:out value="${name}" />
				住所:<c:out value="${address}" />
				番号:<c:out value="${tel}" />
				←選択したデータ<br /><br />

				<form:form modelAttribute="formModel" action="Addressupdated">
						<tr><td></td><td><form:errors path="*" element="div" /></td></tr>
						<tr><td><form:label path="name">名前:</form:label></td>
