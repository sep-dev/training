...
<body>
	...
	<%
		...
		try {
			...
			// SQL()を実行して、結果を得る
			rs = stmt.executeQuery("select * from tbAddress where id = ("
					+ id + ");");
			rs.next();
	%>
	<table>
	...
	<FORM ACTION="Update" method="get">
		...
		<p>
			氏名 ：<input type="text" name="name" size="50" maxlength="20"
				value=<%=rs.getString("name")%>>
		</p>
		<p>
			住所 ：<input type="text" name="address" size="50" maxlength="50"
				value=<%=rs.getString("address")%>>
		</p>
		<p>
			電話番号：<input type="text" name="tel" size="48" maxlength="20"
				value=<%=rs.getString("tel")%>>
		</p>
		<input type="submit" value="更新"> <br /> <input type="reset"
			value="リセット">
	</FORM>
